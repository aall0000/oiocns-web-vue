...
export default class AnyStore extends Object {
    ...
    private constructor() {
        ...
        this.connection = new signalR.HubConnectionBuilder()
            .withUrl("/orginone/anydata/hub").build()
    ...
    public async aggregate(collName: string, options: any, domain: string) {
        if (this.authed.value) {
            return await this.connection.invoke<ResultType>("Aggregate", collName, options, domain)
        }
        return BadRequst
    }
    ...
    private _resubscribed() {
        if (this.authed.value) {
            Object.keys(this._subscribeCallbacks).forEach(async fullKey => {
                ...
                let { success, data } = await this.connection.invoke<ResultType>("Subscribed", key, domain)
                if (success) {
