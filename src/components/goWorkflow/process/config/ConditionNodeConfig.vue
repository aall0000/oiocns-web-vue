  ...
  <div>
    <el-form inline label-width="100px">
      ...
      <el-form-item label="条件组表达式">
        <el-input size="default" v-model="config.expression" placeholder="输入条件组关系表达式  &为与，|为或"/>
        <span class="item-desc">使用表达式构建复杂逻辑，例如: (A & B) | C</span>
      </el-form-item>
    </el-form>
    <div>
      <el-button type="primary" size="default" icon="el-icon-plus" style="margin: 0 15px 15px 0" round @click="addConditionGroup">
        添加条件组
      </el-button>
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      const prioritySortList = computed(() => {
        ...
        console.log(selectedNode.id, node)
        if (node) {
          return node.branchs || []
        }
        return []
