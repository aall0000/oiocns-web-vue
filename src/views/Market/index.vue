<template>
  <div class="market-layout">
    ...
    <div class="market-content box">
      <ul class="box-ul">
        ...
        <li v-show="mode === 'list'">
          <DiyTable ref="diyTable" :hasTitle="true" :tableData="state.ownProductList" :tableHead="state.tableHead">
            <template #operate="scope">
              <el-dropdown trigger="click" @command="(value) => handleCommand('own', value, scope.row)"
        ...
        <el-pagination style="justify-content: end" hide-on-single-page layout="prev, pager, next"
      ...
      <ul class="box-ul">
        ...
        <li class="app-card" v-show="mode === 'card'">
          <ShopCard v-for="item in state.shareProductList" :info="item" :key="item.id" :over-id="item.id">
            <template #icon>
              <HeadImg :name="item.name" :url="item.icon || appImg" :imgWidth="48" :createUser="item.createUser"
                :limit="1" :isSquare="false" />
            </template>
            <template #rightIcon>
              <el-dropdown trigger="click" @command="(value) => handleCommand('own', value, item)"
                placement="left-start">
        ...
        <li v-show="mode === 'list'">
          <DiyTable ref="diyTable" :hasTitle="true" :tableData="state.shareProductList" :tableHead="state.tableHead">
            <template #operate="scope">
              <el-dropdown trigger="click" @command="(value) => handleCommand('own', value, scope.row)"
                placement="bottom-end">
                <el-icon :size="18">
  ...
  <el-dialog v-if="personCohortShareVisible" v-model="personCohortShareVisible" custom-class="share-dialog" title="应用分享"
    width="1000px" draggable :close-on-click-modal="false">
