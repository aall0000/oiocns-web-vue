  ...
  <div class="cohortLayout">
    ...
    <div class="cohortLayout-content">
      <div class="cohortLayout-content-left" :style="'width:' + (radio == '1' ? '49%' : '33%')">
        ...
        <el-tree v-if="radio == '1'" ref="leftTree" node-key="id" :data="cascaderTree" :check-strictly="true"
          :default-expand-all="true" show-checkbox @check-change="handleCheckChange" :filter-node-method="filterNode"
          :props="{ class: customNodeClass }" />
        <el-tree v-else ref="leftTree" :key="radio" :data="cascaderTree" :highlight-current="true"
          :expand-on-click-node="false" :props="{ class: customNodeClass }" :default-expand-all="true"
          @node-click="handleNodeClick" :filter-node-method="filterNode" />
      </div>
      <div class="cohortLayout-content-center" style="width: 33%" v-if="radio !== '1'" v-infinite-scroll="load"
        ...
        <el-tree v-if="radio == '2' || radio == '3' || radio == '4'" ref="centerTree" node-key="id"
          :check-strictly="true" :data="state.centerTree" :props="authorityProps" :default-expand-all="true"
          show-checkbox @check-change="centerAuthorClick" />
      </div>
      <div class="cohortLayout-content-right" :style="'width:' + (radio == '1' ? '49%' : '33%')">
        <Author v-if="radio == '1'" @delContent="delContent" :departData="state.departData"></Author>
        <Author v-if="radio == '2'" @delContent="delContentAuth" :departData="state.authorData"></Author>
        <Author v-if="radio == '3'" @delContent="delContentAuth" :departData="state.identitysData"></Author>
        <Author v-if="radio == '4'" @delContent="delContentAuth" :departData="state.personsData"></Author>
      </div>
    </div>
      ...
      case '3':
        ...
        API.company
          ...
          .then((res: ResultType) => {
            ...
            result.forEach((item) => {
              item.disable = !authority.IsApplicationAdmin([
                item.belongId, node.data.belongId
              ])
            })
            if (load == true) {
...
const isAuthAdmin = (node: any) => {
  node.disabled = !authority.IsApplicationAdmin([
    node.data.id,
    node.data.belongId
  ])
  if (node.children) {
    for (const children of node.children) {
      isAuthAdmin(children)
    }
  }
  //判断是否有操作权限
  return node
