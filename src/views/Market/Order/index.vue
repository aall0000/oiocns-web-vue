<template>
  <MarketCard>
    <template #right>
      <el-select v-model="statusvalue" filterable placeholder="订单状态" clearable @change="getTableList(searchType)"
          ...
          <div style="margin-left: 50px">
            <DiyTable ref="diyTableDetail" :tableData="props.row.details" :tableHead="state.tableHeadBuyDetail"
              ...
              <template #operate="scope">
                ...
                <el-button link small type="danger" class="btn" v-show="scope.row.status < 102"
                  @click="cancelBuy(scope.row.id)">
                  取消订单
                </el-button>
                <el-button link small type="primary" class="btn" v-show="scope.row.status == 102"
      ...
      <DiyTable v-else ref="diyTable" :hasTitle="true" :tableData="state.orderMessage.list" :tableHead="state.tableHeadSell"
        ...
        <template #operate="scope">
          <el-space>
            ...
            <el-button link small type="primary" v-show="scope.row.status < 102 && scope.row.merchandise"
              @click="delivery(scope.row.id)">
              确认交付
            </el-button>
            <el-button link small type="danger" v-show="scope.row.status < 102" @click="cancelSell(scope.row.id)">
              取消订单
            </el-button>
          </el-space>
        </template>
